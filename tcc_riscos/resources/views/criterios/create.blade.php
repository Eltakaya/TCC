                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('criterios.store') }}" method="POST">
                        ...
                        <div class="mt-4">
                            ...
                            <select name="tipo_riscos_id" id="tipo_riscos_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach($tipoRiscos as $tipo)
                                    <option value="{{ $tipo->id }}" {{ old('tipo_riscos_id') == $tipo->id ? 'selected' : '' }}>
                                        {{ $tipo->risco }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="descricao" id="descricao" rows="3" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('descricao') }}</textarea>
                            <x-input-error :messages="$errors->get('descricao')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="peso" :value="__('Peso (Opcional)')" />
                            <x-text-input id="peso" class="block mt-1 w-full" type="number" step="0.01" name="peso" :value="old('peso')" />
